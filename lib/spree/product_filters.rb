...
module Spree
  module Core
    ...
    module ProductFilters
      ...
      #
      Spree::Product.add_search_scope :my_price_range_any do |*opts|
        conds = opts.map { |o| Spree::Core::ProductFilters.my_price_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
      ...
      def self.my_price_filter
        v = Spree::Price.arel_table
        conds = [[I18n.t('spree.under_price', price: format_price(200)), v[:amount].lteq(200)],
                 ["#{format_price(200)} - #{format_price(300)}", v[:amount].in(200..300)],
                 ["#{format_price(300)} - #{format_price(500)}", v[:amount].in(300..500)],
                 [I18n.t('spree.or_over_price', price: format_price(500)), v[:amount].gteq(500)]]
        {
          name:   I18n.t('spree.price_range'),
          scope:  :my_price_range_any,
      ...
      Spree::Product.add_search_scope :my_brand_any do |*opts|
        conds = opts.map { |o| ProductFilters.my_brand_filter[:conds][o] }.reject(&:nil?)
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
